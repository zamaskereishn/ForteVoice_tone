...
Enhanced with memory management to handle large datasets efficiently.
...
import soundfile
import tqdm
from datasets import Audio, Dataset, IterableDataset, load_dataset, disable_caching, enable_caching
from hydra.conf import HydraConf, RunDir
from hydra.core.config_store import ConfigStore
from omegaconf import OmegaConf

# Memory management settings
MEMORY_CHECK_INTERVAL = 100  # Check memory every N samples
MAX_MEMORY_GB = 8.0  # Clear cache when memory usage exceeds this
CACHE_CLEAR_INTERVAL = 1000  # Clear cache every N samples
...
def clear_hf_cache():
    """Clear HuggingFace cache to free memory."""
    try:
        disable_caching()
        enable_caching()
        gc.collect()
...
def build_map_dataset_to_nemo_func(cfg: HFDatasetConversionConfig, basedir):
    ...
    """
    # input is non decoded audio samples with ['audio'] containing path and bytes
    def map_dataset_to_nemo(batch):
        try: 
            # init decoder 
            tmp_decoder = Audio(sampling_rate=cfg.sampling_rate, decode=True)
            ...
            if y.ndim != 1:
                raise ValueError(f"Expected mono, got shape {y.shape}")
            ...
            soundfile.write(audio_filepath, y, samplerate=cfg.sampling_rate, format='wav')

            # batch['duration'] = librosa.get_duration(y=batch['audio']['array'], sr=batch['audio']['sampling_rate'])
            batch['duration'] = librosa.get_duration(y=y, sr=y_sr)        
            return batch
        except Exception as e: 
            import numpy as np
            print(f"Warning: Skipping corrupted audio sample {batch.get('id', 'unknown')}: {e}")
            
            # Create dummy audio (0.1 seconds of silence)
            dummy_audio = np.zeros(int(cfg.sampling_rate * 0.1))
            
            # Create a safe filename for corrupted sample
            safe_id = batch.get('id', 'corrupted').replace('/', '_').replace('\\', '_')
            batch['audio_filepath'] = os.path.join(basedir, f"CORRUPTED_{safe_id}.wav")
            batch['audio_filepath'] = prepare_audio_filepath(batch['audio_filepath'])
            
            # Save dummy audio
            soundfile.write(batch['audio_filepath'], dummy_audio, samplerate=cfg.sampling_rate, format='wav')
            batch['duration'] = 0.1
...
def convert_offline_dataset_to_nemo(
    ...
    # added batched=False 
    dataset = dataset.map(build_map_dataset_to_nemo_func(cfg, basedir), num_proc=num_proc, batched=False)
    ...
    with open(manifest_filepath, 'w') as manifest_f:
        for idx, sample in enumerate(
            ...
            # Memory management - check memory usage periodically
            if idx % MEMORY_CHECK_INTERVAL == 0 and idx > 0:
                memory_gb = get_memory_usage_gb()
                if PSUTIL_AVAILABLE and memory_gb > MAX_MEMORY_GB:
                    print(f"\nMemory usage: {memory_gb:.1f}GB, clearing cache...")
                    clear_hf_cache()
            
            # Periodic cache clearing
            if idx % CACHE_CLEAR_INTERVAL == 0 and idx > 0:
                print(f"\nProcessed {idx} samples, clearing cache...")
                clear_hf_cache()
...
def process_dataset(dataset: IterableDataset, cfg: HFDatasetConversionConfig):
    ...
    Args:
        ...
        cfg: HFDatasetConvertionConfig
    """
    # remove decoding and decode manually 
    # dataset = dataset.cast_column("audio", Audio(cfg.sampling_rate))
    dataset = dataset.cast_column('audio', Audio(cfg.sampling_rate, decode=False))
    ...
    if PSUTIL_AVAILABLE:
        memory_gb = get_memory_usage_gb()
        print(f"Starting processing with {memory_gb:.1f}GB memory usage")
        print(f"Will clear cache when memory exceeds {MAX_MEMORY_GB}GB")
        print(f"Will clear cache every {CACHE_CLEAR_INTERVAL} samples")
...
def main(cfg: HFDatasetConversionConfig):
    ...
    print("Dataset loaded, clearing initial cache...")
    clear_hf_cache()
